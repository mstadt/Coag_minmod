...
TF_vect = logspace(-3,2,6); 
 
% Change parameter values here
p.kF = 1;
p.Ks = 0.1;
p.ep = 0.1;
for i_index = 1:length(TF_vect)

p.TF_VIIa0 = TF_vect(i_index); 
% No IX/VIII dynamics
p.VIII_up = 0; 
p.k10_8 = 0; 
...
%% Run simulation
[t,y] = ode45(@(t,y) mammal_mod_edit(t,y,params,...
                        I1, I2),...
                        tspan, IC, opts_ode);
...
figure(25)
yline(T(id),'--','color',c2,'handlevisibility','off'); 
xline(t(id),'--','color',c2,'handlevisibility','off'); 
